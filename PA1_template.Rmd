...
print(maxInterval)
```
Apparently it's interval number `r maxInterval`.  
...
```
It seems there are `r NASum` missing values in our data set.  
...
The mean steps per interval on weekdays is `r wdMean`.  
The mean steps per interval on weekends is `r weMean`.  
